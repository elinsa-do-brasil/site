...
const arquivo = path.resolve(
  './scripts/comunidades-quilombolas.xlsx'
);
...
const dados = XLSX.utils.sheet_to_json<Record<string, any>>(
  worksheet
);
...
// tenta encontrar automaticamente a coluna do município
const primeiraLinha = dados[0];

const colunaMunicipio = Object.keys(
  primeiraLinha
).find((coluna) => {
  ...
  return (
    ...
    nome.includes('MUNICÍPIO')
  );
});

if (!colunaMunicipio) {
  throw new Error(
    'Não encontrei a coluna de município no Excel.'
  );
}

const comunidadesPorMunicipio = new Map<
  string,
  number
>();

for (const linha of dados) {
  const municipioExcel = normalizar(
    String(linha[colunaMunicipio] || '')
  );

  if (municipiosSet.has(municipioExcel)) {
    comunidadesPorMunicipio.set(
      municipioExcel,
      (comunidadesPorMunicipio.get(
        municipioExcel
      ) || 0) + 1
    );
  }
...
for (const municipio of municipios) {
  ...
  const quantidade =
    comunidadesPorMunicipio.get(chave) || 0;
